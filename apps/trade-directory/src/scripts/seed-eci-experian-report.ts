...
import * as json from './data/eci.json';
...
async function bootstrap() {
  ...
  logger.log(
    `Seeding Experian ${ExperianReportTypeEnum.ENHANCED_COMPANY_INTELLIGENCE} report...`,
  );
  ...
  for (const datum of data) {
    ...
    const experianSeed = new Experian({
      reportType: ExperianReportTypeEnum.ENHANCED_COMPANY_INTELLIGENCE,
      ...
      interestInOtherCompanies: datum['Interest In Other Companies'].map(
        (i) => ({
          localNo: i['Local No'],
          company: i.Company,
          shareholding: i.Shareholding as unknown as string,
...
function organizationTypeConverter(
  ...
  if (organizationTypeValue.includes('SOLE-PROPRIETORSHIP')) {
