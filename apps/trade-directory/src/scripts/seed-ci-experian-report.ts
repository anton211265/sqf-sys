...
import * as json from './data/ci.json';
...
async function bootstrap() {
  ...
  logger.log(
    `Seeding Experian ${ExperianReportTypeEnum.COMPANY_INTELLIGENCE} report...`,
  );
  ...
  for (const datum of data) {
    if (datum['Registration No'] === '') {
      logger.warn(`Registration number is empty, skipping it...`);
      continue;
    }
    logger.log(
    ...
    const experianSeed = new Experian({
      reportType: ExperianReportTypeEnum.COMPANY_INTELLIGENCE,
