// E2E — Customer Portal pass 2: ILO view (sanitized), passkey e-signature
...
//   node apps/customer-portal/scripts/e2e-portal-pass2.mjs
...
const ORG2 = 2;
const ORG_Z = 'E2E Pass2 Client Sdn Bhd';
const CLIENT_EMAIL = 'finance@e2e-pass2-client.example';
const STAFF = ['e2e-p2-admin@test.local', 'e2e-p2-cra2@test.local', 'e2e-p2-cm@test.local'];
...
function teardown() {
  ...
  const appId = ro(`SELECT id FROM application WHERE "applicationNumber"='E2EP2_APP'`);
  if (appId) {
  ...
  td(`DELETE FROM organization_role WHERE name IN ('E2E P2 CRA2','E2E P2 CM') AND "organizationId"=${ORG2}`);
...
async function main() {
  ...
  grant('E2E P2 CRA2', ['risk_offers_view', 'risk_offers_check']);
  grant('E2E P2 CM', ['risk_offers_view', 'risk_offers_approve', 'risk_offers_resolve']);
  for (const [email, roleSql] of [
    ...
    [STAFF[1], `SELECT p.id, r.id FROM person p, organization_role r WHERE p.email='${STAFF[1]}' AND r.name='E2E P2 CRA2' AND r."organizationId"=${ORG2}`],
    [STAFF[2], `SELECT p.id, r.id FROM person p, organization_role r WHERE p.email='${STAFF[2]}' AND r.name='E2E P2 CM' AND r."organizationId"=${ORG2}`],
  ]) {
  ...
  ro(`INSERT INTO application ("organizationId","clientOrganizationName","applicationStatus","applicationNumber","funderOrganizationId","productCode","applicationPayload") VALUES (${orgZ},'${ORG_Z}','SCORED_PASS','E2EP2_APP',${ORG2},'AR','{}')`);
  const appId = parseInt(ro(`SELECT id FROM application WHERE "applicationNumber"='E2EP2_APP'`), 10);
  crm(`INSERT INTO applicant_intake ("funderOrganizationId","applicationId","applicationNumber","organizationId","companyName","productCode","result") VALUES (${ORG2},${appId},'E2EP2_APP',${orgZ},'${ORG_Z}','AR','PASS')`);
