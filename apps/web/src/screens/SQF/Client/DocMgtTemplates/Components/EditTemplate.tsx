...
const EditTemplateModal: FC<IProps> = (props) => {
  const { opened, onClose, onSuccess, templateData } = props;
  const { name, llmProvider, prompt, templateId } = templateData;
  const editTemplateMutation = useEditTemplate();
  ...
  useEffect(() => {
    if (opened) {
      form.setValues({ templateName: name, llmProvider, prompts: prompt });
    }
  }, [opened, templateData]);
    ...
    >
      ...
      <Group justify="flex-end">
        ...
        {activeStep > 0 && (
          <Button
            color="gray"
            disabled={editTemplateMutation.isLoading}
            onClick={prev}
          >
            Back
          </Button>
        ...
        {activeStep === 2 && (
          <Button loading={editTemplateMutation.isLoading} onClick={next}>
            Save
          </Button>
        )}
      </Group>
